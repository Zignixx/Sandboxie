...
#define HOOK_WIN32(func) {                                          \
    ...
    __sys_##func =                                                  \
        (ULONG_PTR)CobraSboxDll_Hook(FuncName, SourceFunc, my_##func, KernelBase);   \
    if (! __sys_##func)                                             \
        hook_success = FALSE;                                       \
    }
    ...
    BOOL FindSystemToken)
{
    ...
    for (i = 0; i < pid_count; ++i) {
        ...
        if (_wcsicmp(image, ProcessName) == 0 &&
            ...
            if (process) {
                if (CobraSboxDll_CheckProcessLocalSystem(process))
                    found = TRUE;
                CloseHandle(process);
            }
...
_FX BOOL my_SetServiceStatus(SERVICE_STATUS_HANDLE hServiceStatus, LPSERVICE_STATUS lpServiceStatus) {
    int i = 0;
    //
    // update service status in CobraSboxDll
    ...
    DWORD dwDesiredAccess)
{
    ...
    if (_wcsicmp(lpServiceName, L"RpcSs") == 0) {
        ...
        // fallback to CobraSboxDll SCM implementation
        ...
        if (CobraSboxDll_IsBoxedService(hService)) {
    ...
    void *ArgVector)
{
    ...
    if (hService == SC_HANDLE_MSISERVER) {
        ...
        ok = CobraSboxDll_StartBoxedService(L"MSIServer", FALSE);

    }
    else if (CobraSboxDll_IsBoxedService(hService)) {
