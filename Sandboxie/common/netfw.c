...
BOOLEAN NetFw_ParseRule(NETFW_RULE* rule, const WCHAR* found_value)
{
    ...
    ULONG action_len = 0;
    const WCHAR* tag_list = CobraSboxDll_GetTagValue(found_value, NULL, &action_value, &action_len, L';');

	NetFw_RuleSetBlockAction(rule, _wcsnicmp(action_value, L"Allow", action_len) != 0);

    const WCHAR* port_value;
    ULONG port_len;
    if (CobraSboxDll_FindTagValuePtr(tag_list, L"Port", &port_value, &port_len, L'=', L';')) {
        for (const WCHAR* port_end = port_value + port_len; port_value < port_end;) {
            const WCHAR* port_str1;
            ULONG port_len1;
            port_value = CobraSboxDll_GetTagValue(port_value, port_end, &port_str1, &port_len1, L',');
    ...
    ULONG ip_len;
    if (CobraSboxDll_FindTagValuePtr(tag_list, L"Address", &ip_value, &ip_len, L'=', L';')) {
        for (const WCHAR* ip_end = ip_value + ip_len; ip_value < ip_end;) {
            const WCHAR* ip_str1;
            ULONG ip_len1;
            ip_value = CobraSboxDll_GetTagValue(ip_value, ip_end, &ip_str1, &ip_len1, L',');
    ...
    WCHAR* prot_value;
    ULONG prot_len;
    if (CobraSboxDll_FindTagValuePtr(tag_list, L"Protocol", &prot_value, &prot_len, L'=', L';')) {
