...
MSG_HEADER *EpMapperServer::EpmapperGetPortNameHandler(MSG_HEADER *msg)
{
    ...
    *pwszServiceName = 0;
    ...
    {
        WCHAR buf[MAX_PATH];
        if (CobraSboxDll_GetStringForStringList(req->wszPortId, boxname, L"RpcPortBindingIfId", buf, sizeof(buf)))
        {
        ...
        else if (CobraSboxDll_GetStringForStringList(req->wszPortId, boxname, L"RpcPortBindingSvc", buf, sizeof(buf)))
        {
