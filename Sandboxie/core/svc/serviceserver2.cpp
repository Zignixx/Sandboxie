        ...
        HANDLE idProcess, const WCHAR *ServiceName, ULONG *pSessionId)
{
    ...
    if (ServiceName) {
        ...
        else 
        {
            ...
            if (DropRights && CobraSboxDll_GetSettingsForName_bool(boxname, exename, L"FakeAdminRights", FALSE))
                DropRights = false;
            ...
            if (DropRights && CobraSboxDll_CheckStringInList(ServiceName, boxname, L"StartService"))
                DropRights = false;
        }
    }
    ...
    const WCHAR *InArgs, WCHAR **OutArgs)
{
    ...
    WCHAR *OutPath = NULL;
    if (CobraSboxDll_RunFromHome(START_EXE, args, &si, NULL))
...
int ServiceServer::RunServiceAsSystem(const WCHAR* svcname, const WCHAR* boxname)
{
    ...
    return CobraSboxDll_CheckStringInList(svcname, boxname, L"RunServiceAsSystem") ? 1 : 0;
}
    ...
    ULONG_PTR pkt_addr, ULONG pkt_len)
{
    ...
    if (ok) {
        ...
        errlvl = 0x41;
        if (CobraSboxDll_RunFromHome(SBIESVC_EXE, cmdline, &si, NULL))
    ...
    HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    WCHAR *txtSandbox = CobraSboxDll_FormatMessage0(MSG_3742);
    y += 20 + height;
    ...
    WCHAR *txtProgram = CobraSboxDll_FormatMessage0(MSG_3743);
    y += 20 + height;
...
{
    WCHAR txt[512];
    wcscpy(txt, CobraSboxDll_FormatMessage0(msgid));
    WCHAR *txt1 = txt;
    while (1) {
...
typedef struct _SECURE_UAC_PACKET {
    ...
    //
    // keep in sync with CobraSboxDll / secure.c
    //
    ...
    HANDLE hClientProcess, void *xpkt, WCHAR **OutAppName)
{
    ...
    //
    // note that CobraSboxDll hooks RtlQueryElevationFlags in order to prevent
    ...
    const WCHAR *cmdline, bool isAdmin)
{
    ...
    LONG_PTR ReturnCode = pUacDialog(
                GetModuleHandle(CobraSboxDll L".dll"),
