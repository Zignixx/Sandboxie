    ...
    PROCESS *proc, HANDLE ProcessHandle, void *InfoBuffer, ULONG InfoLength)
{
    ...
    NTSTATUS status;
    KIRQL irql;
    BOOLEAN SpecialCallFromCobraSboxDll;
    ...
    SpecialCallFromCobraSboxDll = FALSE;
    __try {
        status = STATUS_SUCCESS;
        if (InfoLength == sizeof(PROCESS_ACCESS_TOKEN)) {
            ...
            if ((! Info->Token) && (! Info->Thread))
                SpecialCallFromCobraSboxDll = TRUE;
        }
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        status = GetExceptionCode();
    }
    ...
    if (proc2 && (! proc2->initialized)) {
        ...
        // in CobraSboxDll, and in this case, InfoBuffer should contain zeroes,
        // and SpecialCallFromCobraSboxDll will be set by the code sequenece above
        //

        if (! SpecialCallFromCobraSboxDll) {
            ...
            //
            // the special call from CobraSboxDll is a request to copy the active
...
{
    //
    // the call to SetProcessWindowStation by CobraSboxDll during process
