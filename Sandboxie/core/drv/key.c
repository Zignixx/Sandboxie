...
_FX NTSTATUS Key_MyParseProc_2(OBJ_PARSE_PROC_ARGS_2)
{
    ...
    if (! IsBoxedPath) {
        ...
        // \REGISTRY\MACHINE for MAXIMUM_ALLOWED access even before CobraSboxDll
        // was loaded, so permit those accesses
        //
        // note that since version 3.45, CobraSboxDll takes over the job of
        // loading static import DLLs, however ADVAPI32 is loaded directly
        // by NTDLL even before manipulating static imports.
        //

        if (status == STATUS_ACCESS_DENIED && (! proc->CobraSboxDll_loaded) &&
