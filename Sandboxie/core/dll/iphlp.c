...
_FX BOOLEAN IpHlp_Init(HMODULE module)
{
    ...
    CobraSboxDll_HOOK(IpHlp_,IcmpCreateFile);
    CobraSboxDll_HOOK(IpHlp_,Icmp6CreateFile);
    CobraSboxDll_HOOK(IpHlp_,IcmpCloseHandle);

    CobraSboxDll_HOOK(IpHlp_,IcmpSendEcho);
    CobraSboxDll_HOOK(IpHlp_,IcmpSendEcho2);
    CobraSboxDll_HOOK(IpHlp_,Icmp6SendEcho2);
    if (IcmpSendEcho2Ex) {
        CobraSboxDll_HOOK(IpHlp_,IcmpSendEcho2Ex);
    }

    if (NotifyRouteChange2) {
        CobraSboxDll_HOOK(IpHlp_,NotifyRouteChange2);
    }
    if (CancelMibChangeNotify2) {
        CobraSboxDll_HOOK(IpHlp_,CancelMibChangeNotify2);
    }
...
_FX ULONG_PTR IpHlp_CommonCreate(BOOLEAN ip6)
{
    ...
    rpl = (IPHLP_CREATE_FILE_RPL *)CobraSboxDll_CallServer(&req.h);
    if (! rpl)
                                    ...
                                    BOOLEAN ip6, BOOLEAN ex2)
{
    ...
    rpl = (IPHLP_SEND_ECHO_RPL *)CobraSboxDll_CallServer(&req->h);
