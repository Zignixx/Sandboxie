...
_FX BOOLEAN UserEnv_InitVer(HMODULE module)
{
    ...
    WCHAR str[32];
    if (CobraSboxDll_GetSettingsForName(NULL, Dll_ImageName, L"OverrideOsBuild", str, sizeof(str), NULL))
    ...
    CobraSboxDll_HOOK(UserEnv_, RtlGetVersion);
    CobraSboxDll_HOOK(UserEnv_, GetVersionExW);
    CobraSboxDll_HOOK(UserEnv_, GetVersionExA);
...
_FX BOOLEAN UserEnv_Init(HMODULE module)
{
    ...
    if (module == Dll_KernelBase) {
        ...
        CobraSboxDll_HOOK(UserEnv_,GetAppliedGPOList);
        ...
        CobraSboxDll_HOOK(UserEnv_,RegisterGPNotification);
        CobraSboxDll_HOOK(UserEnv_,UnregisterGPNotification);
    }
