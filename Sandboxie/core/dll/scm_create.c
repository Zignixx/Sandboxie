...
//---------------------------------------------------------------------------
// CobraSboxDll_StartBoxedService
//---------------------------------------------------------------------------


_FX BOOL CobraSboxDll_StartBoxedService(const WCHAR *ServiceName, BOOLEAN WithAdd)
{
    HANDLE hkey;
    SERVICE_STATUS ss;
...
_FX ULONG Scm_StartBoxedService2(const WCHAR *name, SERVICE_QUERY_RPL *qrpl)
{
    ...
    if (_wcsicmp(name, _bits) == 0           ||
        ...
        if (! use_sbiesvc) {
            ...
            if (! CobraSboxDll_RunFromHome(ProcessName, NULL, &si, &pi))
        ...
        si.lpReserved = NULL;
        if (CobraSboxDll_RunFromHome(ProcessName, NULL, &si, NULL)) {
    ...
    SC_HANDLE hService, DWORD dwNumServiceArgs, void *lpServiceArgVector)
{
    ...
    if (Scm_IsBoxedService(ServiceName))
        return CobraSboxDll_StartBoxedService(ServiceName, FALSE);
    ...
    rpl = (SERVICE_START_RPL *)CobraSboxDll_CallServer(&u.req.h);
