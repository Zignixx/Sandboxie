...
//    }                                                                       \
//    __sys_##func =                                                          \
//        (ULONG_PTR)CobraSboxDll_Hook(FuncName, SourceFunc, my_##func);           \
//    if (! __sys_##func)                                                     \
//        hook_success = FALSE;                                               \
//    }
//
...
_FX BOOLEAN Scm_SetupMsiHooks()
{
    ...
    CobraSboxDll_HOOK(Scm_, CreateWaitableTimerW);


    // MSIServer without system - fake running as system
    if (!CobraSboxDll_CheckProcessLocalSystem(GetCurrentProcess()))
    {
