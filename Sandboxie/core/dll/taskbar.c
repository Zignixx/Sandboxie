...
_FX BOOLEAN Taskbar_Init(HMODULE module)
{
    ...
    if (SHGetPropertyStoreForWindow) {
        CobraSboxDll_HOOK(Taskbar_,SHGetPropertyStoreForWindow);
    }
...
_FX BOOLEAN Taskbar_Init_2(HMODULE module)
{
    ...
    if (SetCurrentProcessExplicitAppUserModelID) {
        CobraSboxDll_HOOK(Taskbar_,SetCurrentProcessExplicitAppUserModelID);
    }

    if (GetCurrentProcessExplicitAppUserModelID) {
        CobraSboxDll_HOOK(Taskbar_,GetCurrentProcessExplicitAppUserModelID);
    }
...
_FX BOOLEAN Taskbar_ShouldOverrideAppUserModelId(void)
{
    ...
    if (path) {
        ...
        if (NtPath) {
            ...
            ULONG mp_flags = CobraSboxDll_MatchPath(L'f', NtPath);
...
_FX void Taskbar_GetProgramName(WCHAR **name, WCHAR **path)
{
    ...
    *path = _path;
    ...
    CobraSboxDll_TranslateNtToDosPath(_path);
    ...
    if (hFile != INVALID_HANDLE_VALUE) {
        ...
        NTSTATUS status = CobraSboxDll_GetHandlePath(hFile, path1, &IsBoxedPath);
...
_FX WCHAR *Taskbar_GetStartExeCommand(const WCHAR *args)
{
    ...
    CobraSboxDll_TranslateNtToDosPath(command + 1);
    wcscat(command, L"\" ");
