...
_FX BOOLEAN RpcRt_Init(HMODULE module)
{
    ...
    CobraSboxDll_HOOK(RpcRt_,RpcBindingInqAuthClientEx);
    ...
    if ( Dll_OsBuild >= 15063) {
        ...
        CobraSboxDll_HOOK(RpcRt_,RpcStringBindingComposeW);
    }
    ...
    CobraSboxDll_HOOK(RpcRt_, RpcBindingFromStringBindingW);
    ...
    CobraSboxDll_HOOK(RpcRt_, RpcBindingFromStringBindingA);
    ...
    CobraSboxDll_HOOK(RpcRt_, RpcBindingCreateW);
    ...
    if (SbieApi_QueryConf(NULL, L"IpcTrace", 0, wsTraceOptions, sizeof(wsTraceOptions)) == STATUS_SUCCESS && wsTraceOptions[0] != L'\0')
    {
        ...
        CobraSboxDll_HOOK(RpcRt_, NdrClientCall);
    ...
    CobraSboxDll_HOOK(RpcRt_, RpcAsyncCompleteCall);
    ...
    const WCHAR* CallingModule, const WCHAR* Identifier, WCHAR* value, ULONG value_size)
{
    ...
    while (1) {
        ...
        ULONG test_len = 0;
        found_value = CobraSboxDll_GetTagValue(found_value, NULL, &test_value, &test_len, L',');
...
WCHAR* GetDynamicLpcPortName(const WCHAR* wszPortId)
{
    ...
    rpl = (EPMAPPER_GET_PORT_NAME_RPL*)CobraSboxDll_CallServer(&req.h);
    ...
    __out RPC_BINDING_HANDLE* OutBinding)
{
    ...
    /*else if (_wcsicmp(StringBinding, L"0497b57d-2e66-424f-a0c6-157cd5d41700@ncalrpc:") == 0) { // Appinfo
        ...
        // CobraSboxDll.dll!Proc_CreateProcessInternalW_RS5
    ...
    if (CallingModule)
    {
        ...
        use_RpcMgmtSetComTimeout = CobraSboxDll_GetSettingsForName_bool(NULL, CallingModule, L"UseRpcMgmtSetComTimeout", use_RpcMgmtSetComTimeout);
        ...
        if (NT_SUCCESS(RpcRt_FindModulePreset(CallingModule, StringBinding, ModulePreset, sizeof(ModulePreset)))) {
            ...
            WCHAR tagValue[96];
            if (CobraSboxDll_FindTagValue(ModulePreset, L"Resolve", tagValue, sizeof(tagValue), L'=', L',')) {
                pwszTempPortName = GetDynamicLpcPortName(tagValue);
            }
            
            if (!pwszTempPortName && CobraSboxDll_FindTagValue(ModulePreset, L"IpcPort", tagValue, sizeof(tagValue), L'=', L',')) {
            ...
            if (CobraSboxDll_FindTagValue(ModulePreset, L"TimeOut", tagValue, sizeof(tagValue), L'=', L','))
    ...
    if (CallingModule)
    {
        ...
        if (NT_SUCCESS(RpcRt_FindModulePreset(CallingModule, StringUuid, ModulePreset, sizeof(ModulePreset)))) {
            ...
            if (RPC_PROTSEQ_LRPC == Template->ProtocolSequence && !Template->StringEndpoint)
            {
                ...
                if (!Template->StringEndpoint && CobraSboxDll_FindTagValue(ModulePreset, L"IpcPort", tagValue, sizeof(tagValue), L'=', L',')){
