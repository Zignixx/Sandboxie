        ...
        if (j) {
            ...
            for (j = 0; j < len; ++j) {
                ...
                if (GetModuleBaseName(hProcess, mods[j], name, 60)) {
                    ...
                    if (_wcsicmp(name, CobraSboxDll L".dll") == 0) {
...
_FX ULONG Kmd_DoWindow(WCHAR *text, PGM *pgms, BOOLEAN silent)
{
    ...
    hStatic = CreateWindowEx(
        i, L"STATIC", CobraSboxDll_FormatMessage0(8106),
    ...
    wcscpy(text, CobraSboxDll_FormatMessage0(3001));
    ...
    wcscpy(text, CobraSboxDll_FormatMessage0(3002));
    for (i = 0; i < wcslen(text); ++i)
        if (text[i] == L'&')
            wmemmove(text + i, text + i + 1, wcslen(text + i));
